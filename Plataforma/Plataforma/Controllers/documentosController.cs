...
namespace Plataforma.Controllers
{
    ...
    public class documentosController : Controller
    {
        ...
        // GET: documentos
        public ActionResult Index(string nombre, int? curso, int? unidad, int? nivel, int? tipo)
        {
        ...
        public ActionResult VerDocumento(int? id, int? idCurso, int? idGrupo)
        {
            if (Session["usuario"] != null)
            {
                ...
                if (usuarioSesion.cursos.Where(c => c.documentos_curso.Where(dc => dc.id_documento == id).ToList().Count > 0).ToList().Count > 0)
                {
                    ...
                    {
                        db.documentos.Find(id).contador_visitas = 0;
                    }
                    db.documentos.Find(id).contador_visitas += 1;
